...
# ─────────────────────────── main function ────────────────────────────────────
def cnn_lstm_classifier(df, pred_prices, symbol=None, force_retrain=False):
    """
    Train a CNN-LSTM BUY/SELL/HOLD classifier on technical indicators,
    then predict one signal per forecast day.

    CNN layers extract local temporal patterns from the feature window,
    then an LSTM layer learns sequential dependencies across those patterns.
    ...
    print(f"\n  [CNN-LSTM Classifier] Label distribution: {label_dist}")

    # ── 4. Weight cache lookup ───────────────────────────────────────────────
    time_step = CLASSIFIER_TIME_STEP

    cache_sig = model_registry.build_signature('CNN-LSTM-CLF', df, {
        'time_step':    time_step,
        ...
        'filters':      [64, 32],
        'kernel':       3,
        'arch':         'cnn-lstm-classifier',
    })
    cached_model, cached_scaler, cache_status, _meta = model_registry.load(
        symbol, 'CNN-LSTM-CLF', cache_sig, force_retrain=force_retrain
    )
    ...
    print(f"  [CNN-LSTM Classifier] Total sequences: {len(X_seq)}")
    ...
    print(f"  [CNN-LSTM Classifier] Train: {len(Xtr)}  Val: {len(Xval)}")
    print(f"  [CNN-LSTM Classifier] Class weights — SELL:{cw[0]:.2f}  HOLD:{cw[1]:.2f}  BUY:{cw[2]:.2f}\n")

    # ── 8. Build / load CNN-LSTM model ───────────────────────────────────────
    # Conv1D extracts local feature patterns across the time window,
    # then LSTM captures long-range sequential dependencies on those patterns.
    ...
    if cache_status == 'miss':
        model = Sequential([
            ...
            Dropout(CLASSIFIER_DROPOUT),
            KerasLSTM(CLASSIFIER_LSTM_UNITS_1, return_sequences=True),
            Dropout(CLASSIFIER_DROPOUT),
        ...
        if cache_status == 'warm':
            print("  [Cache] Warm-starting CNN-LSTM classifier on newest sequences…")
            model_registry.warm_start(model, X_seq, to_categorical(y_seq, num_classes=3),
                                      batch_size=CLASSIFIER_BATCH_SIZE,
                                      class_weight=cw_dict)
        else:
            print("  [Cache] Using cached CNN-LSTM classifier weights as-is (no training).")

    if cache_status != 'fresh':
        model_registry.save(symbol, 'CNN-LSTM-CLF', model, feat_scaler, cache_sig)
    ...
    for i in range(FORECAST_DAYS):
        ...
        print(f"  [CNN-LSTM Classifier] Day {i+1} probs — "

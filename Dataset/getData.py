...
def getDataFrame(SYMBOL):
    def decorate(func):
        def decorated(*args,**kwargs):
            ...
            df = capital_market.price_volume_and_deliverable_position_data(symbol=SYMBOL, from_date=from_date, to_date=to_date)
            df = df[['Date', 'ClosePrice','PrevClose', 'HighPrice','LowPrice']]
            df = df.rename(columns={'ClosePrice': 'close', 'PrevClose': 'prev_close', 'HighPrice': 'high', 'LowPrice': 'low'})
            ...
            details = {'scheme_name': ticker_info['NAME OF COMPANY'].values[0], 'scheme_code': str(SYMBOL)}
